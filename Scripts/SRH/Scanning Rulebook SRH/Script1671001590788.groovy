...
WebUI.callTestCase(findTestCase('SRH/AdUPN'), [:], FailureHandling.STOP_ON_FAILURE)
...
// Fetch and write policy on Excel file
try {
def NewSRHPolicy = WebUI.getText(findTestObject('Object Repository/Scanning/GetinterviewDetails/Get Policy'))
	sheet.getRow(8).createCell(1).setCellValue(NewSRHPolicy);
	sheet1.getRow(8).createCell(3).setCellValue("Pass");
	sheet1.getRow(9).createCell(3).setCellValue("Pass");
}
catch(Exception E) {
	sheet1.getRow(8).createCell(3).setCellValue("Fail");
	sheet1.getRow(9).createCell(3).setCellValue("Fail");
	file.close()
